...
We can use the route that we retrieved with our last command to evaluate our data against the rules of our `coffeeshop-service`. Execute the following commmand using cURL (replace the host and port with the information retrieved from your route):
