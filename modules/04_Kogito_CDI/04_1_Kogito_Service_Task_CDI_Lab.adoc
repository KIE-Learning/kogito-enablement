...
```
$ kogito new-project service-task-cdi
```
...
Go back to your terminal and execute the following command to create a new Kogito service definition for our `service-task-cdi` service:

```
$ kogito deploy-service service-task-cdi
```
...
image:images/service-task-cdi-kogito-service.png[Service Task CDI Kogito Service]
...
service-task-cdi           Source            0
service-task-cdi-binary    Source   Binary   0
service-task-cdi-builder   Source   Binary   0
```

... and as we can see, a `service-task-cdi-binary` build configuration has been created.
...
Because we're using a binary build approach, we first need to create our runnable JAR file. Make sure that you're in the root directory of your `service-task-cdi` project (the directory that contains your projects `pom.xml` fil), and run the following Maven command to build your runnable Kogito JAR file:

```
$ mvn clean package -DskipTests
```

After the buils has finished, there will be a file called `service-task-cdi-1.0-SNAPSHOT-runner.jar` in your `target` directory:
...
```
$ oc start-build service-task-cdi-binary --from-dir=target/
```
...
```
$ oc logs -f build/service-task-cdi-binary-1
```
...
service-task-cdi   2          1         1         config,image(service-task-cdi:latest)
```
...
keycloak-operator-metrics   ClusterIP   172.30.61.75     <none>        8383/TCP,8686/TCP   63m
kogito-operator-metrics     ClusterIP   172.30.71.147    <none>        8383/TCP,8686/TCP   63m
service-task-cdi            ClusterIP   172.30.188.229   <none>        8080/TCP            28m
```
...
service-task-cdi   service-task-cdi-service-task-cdi.apps-crc.testing          service-task-cdi   http                 None
```
...
```
$ curl -X POST "http://service-task-cdi-service-task-cdi.apps-crc.testing/text_processor" -H "accept: application/json" -H "Content-Type: application/json" -d "{ \"mytext\": \"hello\"}"

{"id":"c26ddef6-0753-4a72-994f-22d65d66dbbc","mytext":"HELLO"}
```

We can also access the Swagger-UI at http://service-task-cdi-service-task-cdi.apps-crc.testing/swagger-ui/ (again, replace the host and port with the values for your environment).

image:images/service-task-cdi-swagger-ui-openshift.png[Swagger-UI OpenShift]
