...
Go back to your terminal and execute the following command to create a new Kogito service definition for our `airmiles-service`:

```
$ kogito deploy-service airmiles-service
```
...
In the OpenShift Console, navigate to your project. Via the menu on the left-hand side of the screen navigate to _Operatorors -> Installed Operators_. Click on the `Kogito` Operator. In the Kogito Operator screen, click on the `Kogito Service` tab. Observe that an `airmiles-service` Kogito Service has been created.

image:images/airmiles-service-kogito-service.png[Airmiles Service Kogito Service]
...
airmiles-service           Source            0
airmiles-service-binary    Source   Binary   0
airmiles-service-builder   Source   Binary   0
```

... and as we can see, a `airmiles-service-binary` build configuration has been created.
...
In the previous part of this lab we've already created a runner JAR of our application using Maven. We can use this JAR file and send it to the _binary build_. Make sure that you're in the root directory of your `airmiles-service` project (the directory that contains your projects `pom.xml` file and the `target` folder with the runnable JAR), and use your `oc` client to start the binary build:

```
$ oc start-build airmiles-service-binary --from-dir=target/
```
...
You can follow the progress of the build using the following command (given that `airmiles-service-binary-1` is the name of your first build instance. You can get a list with the names of the builds with `oc get build`):

```
$ oc logs -f build/airmiles-service-binary-1
```
...
airmiles-service   2          1         1         config,image(airmiles-service:latest)
```
...
airmiles-service            ClusterIP   172.30.232.59    <none>        8080/TCP            103s
keycloak-operator-metrics   ClusterIP   172.30.62.73     <none>        8383/TCP,8686/TCP   8m59s
kogito-operator-metrics     ClusterIP   172.30.135.190   <none>        8383/TCP,8686/TCP   8m54s
```
...
airmiles-service   airmiles-service-airmiles-service.apps-crc.testing          airmiles-service   http                 None
```
...
```
$ curl -X POST 'http://airmiles-service-airmiles-service.apps-crc.testing/airmiles' -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{ "Status": "GOLD",	"Price": 600}'

{"Status":"GOLD","Airmiles":720.0,"Price":600}
```

We can also access the Swagger-UI at http://airmiles-service-airmiles-service.apps-crc.testing/swagger-ui/ (again, replace the host and port with the values for your environment).

image:images/airmiles-service-swagger-ui-openshift.png[Swagger-UI OpenShift]
