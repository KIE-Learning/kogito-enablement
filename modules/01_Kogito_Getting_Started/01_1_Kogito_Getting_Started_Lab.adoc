...
In the OpenShift Console, navigate to your project. Via the menu on the left-hand side of the screen navigate to _Operators -> Installed Operators_. Click on the `Kogito` Operator. In the Kogito Operator screen, click on the `Kogito Service` tab. Observe that a `getting-started` Kogito Service has been created.
